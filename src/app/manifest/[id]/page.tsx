...
function ManifestDetailPage({ wishId, isSignedIn }: { wishId: string; isSignedIn: boolean }) {
			...
			{confirmDelete && (
				<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={() => setConfirmDelete(false)}>
					<div className="bg-white dark:bg-[#111] border border-[#e0e0e0] dark:border-[#2a2a2a] rounded-2xl p-6 max-w-sm w-full shadow-xl" onClick={e => e.stopPropagation()}>
						...
						<p className="mt-1 text-sm text-[#666] dark:text-[#555]">&ldquo;{wish.text}&rdquo; will be permanently removed from Manifest.</p>
...
function AuthManifestDetailPage({ wishId }: { wishId: string }) {
	...
	return <ManifestDetailPage wishId={wishId} isSignedIn={!!isSignedIn} />;
}
...
export default function ManifestDetailRoute() {
	...
	if (!hasClerk) return <ManifestDetailPage wishId={wishId} isSignedIn={false} />;
	return <AuthManifestDetailPage wishId={wishId} />;
}