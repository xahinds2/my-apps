...
import { buildSearchFilter } from "@/features/wish/productQuery";

const HISTORY_LIMIT = 180; // one entry per day × ~6 months
const SIX_MONTHS_MS = 180 * 24 * 60 * 60 * 1000;

// POST /api/wishes/:id/snapshot
// Queries the Product collection for the wish text, appends the cheapest price
// to the wish price history, and returns the current snapshot plus the 6-month low.
export async function POST(
  ...
  try {
    ...
    const { filter, terms } = buildSearchFilter(wish.text, { requireDisplayable: false });
    filter.price = { $exists: true, $gt: 0 };
