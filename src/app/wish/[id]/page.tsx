...
interface WishLink { url: string; label?: string; }
interface Wish {
...
function WishDetailPage({ wishId, isSignedIn }: { wishId: string; isSignedIn: boolean }) {
  ...
  const fetchWish = useCallback(async () => {
    ...
    try {
      if (!isSignedIn || wishId.startsWith('local-')) {
        ...
        setWish(all.find(w => getId(w) === wishId) || null);
        return;
      }
      const res = await fetch(`/api/wishes/${wishId}`);
      const json = await res.json();
      setWish(json.data || null);
    } catch {
      setWish(loadStorage().find(w => getId(w) === wishId) || null);
  ...
  const patchField = useCallback(async (fields: Partial<Wish>) => {
    if (!wish) return;
    const updated = { ...wish, ...fields };
    setWish(updated);
    if (!isSignedIn || wishId.startsWith('local-')) {
      saveStorage(loadStorage().map(w => getId(w) === wishId ? updated : w));
      return;
    }
    const res = await fetch(`/api/wishes/${wishId}`, {
      method: 'PATCH',
    ...
    const json = await res.json();
    if (json.data) setWish(json.data);
  }, [wish, wishId, isSignedIn]);

  const saveName = () => {
    const text = editName.trim();
    if (text && text !== wish?.text) patchField({ text });
  ...
  const addLink = async () => {
    ...
    if (!wish || !isValidUrl(url)) return;
    setSavingLink(true);
    const newLinks = [...(wish.links || []), { url, label: getStoreLabel(url) }];
    setLinkInput('');
  ...
  const deleteWish = async () => {
    if (!wish) return;
    if (!isSignedIn || wishId.startsWith('local-')) {
      saveStorage(loadStorage().filter(w => getId(w) !== wishId));
    } else {
    ...
    <div className="min-h-screen flex flex-col">
      <header className="sticky top-0 z-40 w-full border-b border-[#e0e0e0] dark:border-[#222] bg-white/90 dark:bg-black/90 backdrop-blur-md">
        <div className="max-w-2xl mx-auto px-6 h-14 flex items-center justify-between">
          <Link href="/wish" className="flex items-center gap-2 text-sm text-[#999] hover:text-[#0a0a0a] dark:hover:text-white transition"><ArrowLeft className="h-4 w-4" /> Wishlist</Link>
          <AuthButton />
        </div>
      </header>
  ...
  if (!wish) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-[#666] dark:text-[#555] text-sm">Wish not found</p>
      <Link href="/wish" className="text-xs text-[#999] hover:text-[#0a0a0a] dark:hover:text-white transition flex items-center gap-1"><ArrowLeft className="h-3 w-3" /> Back to wishlist</Link>
    </div>
  );
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <header className="sticky top-0 z-40 w-full border-b border-[#e0e0e0] dark:border-[#222] bg-white/90 dark:bg-black/90 backdrop-blur-md">
        <div className="max-w-2xl mx-auto px-6 h-14 flex items-center justify-between">
          <Link href="/wish" className="flex items-center gap-2 text-sm text-[#999] hover:text-[#0a0a0a] dark:hover:text-white transition">
            <ArrowLeft className="h-4 w-4" /> Wishlist
          </Link>
          <div className="flex items-center gap-3">
            <Link href="/wish" className="hover:opacity-70 transition"><ShoppingBag className="h-4 w-4 text-violet-400" /></Link>
            <AuthButton />
          </div>
        </div>
      </header>
      ...
      <main className="flex-grow max-w-2xl mx-auto w-full px-6 py-8 space-y-8">
        ...
        >
          ...
          <div className="absolute inset-0 z-30 bg-black/0 group-hover:bg-black/20 transition flex items-center justify-center">
            <span className="opacity-0 group-hover:opacity-100 transition text-white text-xs font-medium bg-black/60 px-3 py-1.5 rounded-full">
              {wish.image ? 'Change image' : 'Add image URL'}
            </span>
          </div>
          {/* Priority badge */}
          {wish.priority && (
            <span className="absolute top-3 left-3 z-30 text-xl leading-none drop-shadow">
              {PRIORITY_OPTIONS.find(p => p.value === wish.priority)?.icon}
            </span>
          )}
          {/* Status badge */}
          {isBought && (
          ...
          <div className="flex-grow min-w-0">
            {isEditingName ? (
              ...
              <div className="flex items-center gap-2 group/name">
                <h1 className={`text-xl font-bold tracking-tight ${isBought ? 'text-[#777] dark:text-[#666]' : 'text-[#0a0a0a] dark:text-white'}`}>{wish.text}</h1>
                {isBought && <Check className="h-5 w-5 text-emerald-500 shrink-0" />}
                <button onClick={() => { setEditName(wish.text); setIsEditingName(true); }}
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2 flex-wrap">
            {PRIORITY_OPTIONS.map(opt => (
              <button
                key={opt.value}
                onClick={() => patchField({ priority: wish.priority === opt.value ? undefined : opt.value })}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-sm font-medium transition ${wish.priority === opt.value ? opt.active : opt.inactive + ' hover:border-[#999] dark:hover:border-[#555]'}`}
              >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            {isEditingBudget ? (
              ...
              <button
                onClick={() => { setBudgetInput(wish.budget || ''); setIsEditingBudget(true); }}
              ...
              >
                {wish.budget ? <span className="font-medium text-violet-600 dark:text-violet-400">₹ {wish.budget}</span> : <span className="text-[#bbb] dark:text-[#444]">Set a budget...</span>}
              </button>
            )}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {TIMELINE_OPTIONS.map(opt => (
                <button
                  key={opt}
                  onClick={() => patchField({ timeline: wish.timeline === opt ? undefined : opt })}
                  className={`text-xs px-2.5 py-1 rounded-full border transition ${wish.timeline === opt
        ...
        <div className="space-y-1.5">
          ...
          {isEditingNote ? (
            ...
            <button
              onClick={() => { setNoteInput(wish.note || ''); setIsEditingNote(true); }}
            ...
            >
              {wish.note
                ? <span className="text-[#444] dark:text-[#aaa] whitespace-pre-wrap">{wish.note}</span>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[11px] text-[#bbb] dark:text-[#333]">{wish.links.length} {wish.links.length === 1 ? 'link' : 'links'}</span>
          </div>

          {wish.links.length === 0 && !isAddingLink && (
...
function AuthDetailPage({ wishId }: { wishId: string }) {
  ...
  return <WishDetailPage wishId={wishId} isSignedIn={!!isSignedIn} />;
}

const hasClerk = !!process.env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY;

export default function WishDetailRoute() {
  const params = useParams();
  const wishId = params?.id as string;
  if (!wishId) return null;
  if (!hasClerk) return <WishDetailPage wishId={wishId} isSignedIn={false} />;
  return <AuthDetailPage wishId={wishId} />;
}
