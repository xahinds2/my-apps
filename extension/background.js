...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (message.type === "SAVE_PRODUCTS") {
    ...
    const pageUrl = typeof source === "string" ? source : source.pageUrl;

    saveProducts(message.products, source, message.schemaVersion).then((result) => {
      sendResponse({ ok: true, ...result });
      chrome.action.setBadgeText({ text: result.saved > 0 ? String(result.saved) : "" });
      chrome.action.setBadgeBackgroundColor({ color: "#7c3aed" });
      setTimeout(() => chrome.action.setBadgeText({ text: "" }), 3000);
      appendHistory({ store, count: message.products.length, titles, source: pageUrl, diagnostics });
...
async function saveProducts(products, source, schemaVersion) {
  ...
  const res = await fetch(`${apiUrl}/api/products`, {
    ...
    body: JSON.stringify({ products, source, schemaVersion: schemaVersion || 2 }),
  });
